...
export default function Nav() {
  ...
  //         }
  //         catch(err){
  //           console.error("handle search items data:",result.data)
  //         }}
  ...
  const handleSearchItems = async () => {
    ...
    try {
      const result = await axios.get(
        `${serverUrl}/api/item/search-item`,
        {
          params: {
            query: query,
            city: currentCity,
    ...
    fixed top-0 z-[9999] bg-[#fff9f6]">
      ...
      bg-[#ff4d2d]/10 text-[#ff4d2d]" onClick={() => navigate("/add-item")}>
        ...
        bg-[#ff4d2d]/10 text-[#ff4d2d] text-sm font-medium cursor-pointer" onClick={() => navigate("/my-orders")} >
            ...
            {/* small screen */}
            <button className="flex lg:hidden   text-[#ff4d2d] rounded-lg cursor-pointer" onClick={() => navigate("/my-orders")}>
